...
class ProviderGameUiTest {
    ...
    fun `detects a local installer and does not treat ready jobs as downloadable`() {
        ...
        assertTrue(ProviderGameUi.isInstalled(ready, item, root))
        assertTrue(ProviderGameUi.isInstalled(null, item, root))
        assertTrue(!ProviderGameUi.canInstall(ready, item))
        ...
        val resolving = ready.copy(state = TransferState.RESOLVING)
        assertTrue(!ProviderGameUi.isBusy(resolving))
        assertTrue(ProviderGameUi.canRestart(resolving))
